...
public class PhotoCommentDetailActivity extends KXActivity {
	...
	private PhotoDetailResult mPhotoDetailResult;
	...
	private void setListener() {
		mBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bundle.putInt("count",
						mPhotoDetailResult.getComment_count());
		...
		mLike.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!mIsLike) {
					...
					CallService.setPhotoLike(mPhotoDetailResult.getId());
					mPhotoDetailResult.setLike_count(mPhotoDetailResult.getLike_count() + 1);
					if (mPhotoDetailResult.getLike_count() == 0) {
						mHeadContent.setText("我觉得这个挺赞的");
					} else {
						mHeadContent.setText("我和"
								+ mPhotoDetailResult.getLike_count() + "个人觉得这个挺赞的");
					}
				}
			}
		});
	...
	private void init() {
		...
		mPhotoDetailResult =  (PhotoDetailResult)getIntent().getSerializableExtra("result");
		// 获取照片内容数据
		...
		if (mPhotoDetailResult.getLike_count() == 0) {
			...
			mHeadContent.setText(mPhotoDetailResult.getLike_count() + "个人觉得这个挺赞的");
		}
		if (mPhotoDetailResult.getComment_count() == 0) {
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case ActivityForResultUtil.REQUESTCODE_PHOTOREPLY:
			if (resultCode == RESULT_OK) {
				...
				result.setNickname(StorageUtil.getString(PhotoCommentDetailActivity.this, "nickname"));
				result.setContent(bundle.getString("content"));
				result.setTime("刚刚");
				result.setAlbum_id(mPhotoDetailResult.getAlbum_id());
				result.setPhoto_id(photoId);
				result.setDiary_id(0);
				result.setIs_reply(1);
				result.setAvatar(StorageUtil.getString(PhotoCommentDetailActivity.this, "mAvatar"));
				...
				mPhotoDetailResult.setComment_count(mPhotoDetailResult.getComment_count() + 1);
			}
			break;
		// 照片的评论
