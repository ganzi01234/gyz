 ...
 * 
 */
public class DiaryCommentDetailActivity extends KXActivity {
	...
	private Diary mDiaryDetailResult;
	...
	private void setListener() {
		mBack.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				bundle.putInt("count",
						mDiaryDetailResult.getComment_count());
		...
		mLike.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!mIsLike) {
					...
					CallService.setDiaryLike(mDiaryDetailResult.getId());
					if (mDiaryDetailResult.getLike_count() == 0) {
						mHeadContent.setText("我觉得这个挺赞的");
					} else {
						mHeadContent.setText("我和"
								+ mDiaryDetailResult.getLike_count() + "个人觉得这个挺赞的");
					}
				}
			}
		});
		mComment.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				startActivityForResult(new Intent(
						DiaryCommentDetailActivity.this,
	...
	private void init() {
		...
		mDiaryDetailResult =  (Diary)getIntent().getSerializableExtra("result");
		// 获取照片内容数据
		...
		if (mDiaryDetailResult.getLike_count() == 0) {
			...
			mHeadContent.setText(mDiaryDetailResult.getLike_count() + "个人觉得这个挺赞的");
		}
		if (mDiaryDetailResult.getComment_count() == 0) {
		...
		try {
			JSONObject object = new JSONObject(json);
				JSONArray commentArray = object.getJSONArray(
						"data");
				mCommentResult = new ArrayList<CommentResult>();
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		case ActivityForResultUtil.REQUESTCODE_PHOTOREPLY:
			if (resultCode == RESULT_OK) {
				...
				result.setNickname(StorageUtil.getString(DiaryCommentDetailActivity.this, "nickname"));
				result.setContent(bundle.getString("content"));
				result.setDiary_id(mDiaryDetailResult.getId());
				result.setTime("刚刚");
				result.setAlbum_id(0);
				result.setPhoto_id(0);
				result.setIs_reply(1);
				result.setAvatar(StorageUtil.getString(DiaryCommentDetailActivity.this, "mAvatar"));
				...
				mDiaryDetailResult.setComment_count(mDiaryDetailResult.getComment_count() + 1);
			}
			break;
		// 照片的评论
		...
		public View getView(final int position, View convertView,
			...
			if (convertView == null) {
				convertView = LayoutInflater.from(
						DiaryCommentDetailActivity.this).inflate(
