...
public class DiaryDetailActivity extends KXActivity {
	...
	private Diary mDiaryResult; // 日记的具体内容
		...
		mComment.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				intent.setClass(DiaryDetailActivity.this,
						DiaryCommentDetailActivity.class);
				intent.putExtra("diaryId", mDiaryResult.getId());
				intent.putExtra("result", (Serializable)mDiaryResult);
//				intent.putExtra("result", mMorePhotoResults.get(mCurrentPosition));
	...
	private void init() {
		...
		mDiaryResult = (Diary)getIntent().getSerializableExtra("result");
